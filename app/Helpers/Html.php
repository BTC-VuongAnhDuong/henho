	...
	static function select($options, $name, $attr, $key, $label, $selected = null, $id=null,$blank_select = ''){
		...
		foreach ($options as $i=>$object){
			...
			if($key)
				...
				$html .= '<option value="'.$object.'" '.self::selected($object, $selected,false).' >'.$label_html.'</option>';
		}
		$html .= '</select>';
		return $html;
	}
			...
			<?php if($multi){
				...
				<?php }
					...
					<div class="upload-image-section">
						<input id="upload_image_button" type="button" class="upload_image_button button" value="<?php _e( 'Upload image' ); ?>" />
						<div class='image-preview-wrapper'>
							<img class='image-preview' src='<?php echo $src?>' style='max-height: 100px; width: 100px;'>
						</div>
